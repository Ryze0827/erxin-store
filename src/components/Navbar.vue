  ...
  <nav
    ...
    <div class="container mx-auto px-4 flex items-center justify-between gap-4">
      ...
      <div class="flex items-center shrink-0 space-x-2 lg:space-x-4">
        ...
        <button
          type="button"
          class="hidden lg:inline-flex theme-nav-link items-center justify-center p-2 min-w-[44px] min-h-[44px]"
          :aria-label="themeToggleLabel"
          :title="themeToggleLabel"
          @click="toggleTheme"
        >
          <svg v-if="isDark" class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M12 3v1.5m0 15V21m9-9h-1.5M4.5 12H3m15.364-6.364l-1.061 1.061M6.697 17.303l-1.061 1.061m12.728 0l-1.061-1.061M6.697 6.697L5.636 5.636M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
          </svg>
          <svg v-else class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M21 12.79A9 9 0 1111.21 3a7 7 0 009.79 9.79z" />
          </svg>
        </button>
        ...
        <router-link v-if="showAuthLinks && !userAuthStore.isAuthenticated" to="/auth/login"
          class="hidden lg:inline-flex theme-nav-link items-center gap-1.5 whitespace-nowrap">
          <svg class="w-4 h-4 shrink-0 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="showMobileMenu"
        ...
        <div class="p-5 space-y-1">
          ...
          <div class="mt-4 pt-4 border-t theme-border">
            <button
              type="button"
              class="mb-3 w-full text-left px-4 py-3 rounded-xl theme-nav-link text-sm min-h-[44px] flex items-center justify-between"
              @click="toggleTheme"
            >
              <span class="flex items-center gap-3">
                <svg v-if="isDark" class="w-5 h-5 shrink-0 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M12 3v1.5m0 15V21m9-9h-1.5M4.5 12H3m15.364-6.364l-1.061 1.061M6.697 17.303l-1.061 1.061m12.728 0l-1.061-1.061M6.697 6.697L5.636 5.636M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
                <svg v-else class="w-5 h-5 shrink-0 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.75" d="M21 12.79A9 9 0 1111.21 3a7 7 0 009.79 9.79z" />
                </svg>
                {{ themeToggleLabel }}
              </span>
              <span class="text-xs theme-text-muted">{{ isDark ? t('navbar.darkTheme') : t('navbar.lightTheme') }}</span>
            </button>
...
const showAuthLinks = computed(() => appStore.config?.registration_enabled !== false)

const themeToggleLabel = computed(() => {
  return isDark.value ? t('navbar.switchToLight') : t('navbar.switchToDark')
})
